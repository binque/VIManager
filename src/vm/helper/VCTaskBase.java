...
public class VCTaskBase {
    ...
    protected VCClientSession vcClientSession = new VCClientSession();
    protected VimPortType vimPort;
    protected ServiceContent serviceContent;

    protected void init() throws InvalidLoginFaultMsg, NoSuchAlgorithmException, RuntimeFaultFaultMsg, InvalidLocaleFaultMsg, KeyManagementException {
     ...
     */
    boolean getTaskResultAfterDone(ManagedObjectReference task) throws InvalidCollectorVersionFaultMsg, InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
    ...
    Object getDynamicProperty(ManagedObjectReference mor, String propertyName)
            throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg, NoSuchMethodException, InvocationTargetException, IllegalAccessException,
    ...
    private boolean methodExists(Object obj, String methodName, Class[] parameterTypes) throws NoSuchMethodException {
     ...
     */
    private ObjectContent[] getObjectProperties(ManagedObjectReference mor, String[] properties) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg, InvalidLoginFaultMsg, NoSuchAlgorithmException, InvalidLocaleFaultMsg, KeyManagementException {
        if (mor == null) {
            return null;
        }
     ...
     */
    private List<ObjectContent> retrievePropertiesAllObjects(List<PropertyFilterSpec> listpfs) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg, InvalidLoginFaultMsg, NoSuchAlgorithmException, InvalidLocaleFaultMsg, KeyManagementException {
        init();
    ...
    DatastoreSummary getDatastoreNameWithFreeSpace(ManagedObjectReference virtualMachine, int minFreeSpace)
            throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
        DatastoreSummary ds = null;
        List<ManagedObjectReference> datastores = ((ArrayOfManagedObjectReference) VCHelper.entityProps(serviceContent, vimPort, virtualMachine, new String[]{"datastore"}).get("datastore")).getManagedObjectReference();
        for (ManagedObjectReference datastore : datastores) {
            ds = (DatastoreSummary) VCHelper.entityProps(serviceContent, vimPort, datastore, new String[]{"summary"}).get("summary");
